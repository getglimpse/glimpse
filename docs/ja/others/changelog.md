...
- Converter のドロップエリアからテキスト入力欄に切り替え、貼り付けたテキストを変換できるようにした。プラグインは `allowPaste: false` でテキスト入力を無効化できる

- Converter の Results に出力のファイル名とサイズを表示し、各行に保存・コピー・削除、全体に「すべて保存」を追加

### Changed

#### Plugins

- Converter で選択・ドロップ・貼り付けた入力を自動で変換するようにした。プラグインは `execution: "manual"` で「実行」操作を維持できる

- Converter の出力を削除またはクリアするまで Results に保持し、「新規作成」と「上書き」を保存時に適用するようにした。貼り付けたテキストは元ファイルを上書きできない

#### バックエンド内部

- ファイル保存、インデクサーのランタイム、プラグイン保存、設定処理を責務別のモジュールに分割し、関連テストも整理
- パスのアクセス判定、インデックス作成とファイル監視で使う対象ルートの解決、インデックス対象のソースパス表現を共通化
- Markdown プレビュー生成、プラグイン内ファイルの読み込み、Rust テスト用の一時パス生成を共通化

#### フロントエンド内部

- インストール済みプラグイン、リモートプラグイン、プラグイン内部ページを、インストール操作・詳細表示・フォーム・README 表示などの部品に分割
- プラグインランタイムの起動管理から、サンドボックス通信、権限チェック、描画、状態管理を分離。重複していた権限コンテキストの設定とアセット参照の検証を共通化
- 検索フィルターの更新処理を共通化し、フィルターの組み合わせと入力形式を維持
