...
| Converter | 選択・ドロップしたファイルや、貼り付けたテキストを変換する         | Markdown から HTML への変換、テキストファイルの整形 |
...
Converter は選択・ドロップしたファイルを非同期で変換します。コピーしたテキストはドロップエリア右下の切り替えボタンを押し、表示されたテキストエリアで Ctrl+V を押して入力できます。プラグインは `allowPaste: false` で切り替えボタンを隠せます。変換結果は Results に残り、各行で保存・コピー・削除を選べます。「すべて保存」は未保存の結果をまとめて保存します。「新規作成」と「上書き」は保存時に適用されます。上書きには元ファイルへの権限と、入力と出力の一対一の対応が必要です。貼り付けたテキストの結果は上書きできません。プラグインが `execution: "manual"` を指定した場合は、変換前の「実行」操作を維持できます。
