...
Converter では、選択またはドロップしたファイルをまとめて確認してから実行ボタンで処理します。Glimpse 本体がすべての Converter タブのドロップエリア右上に「新規作成」と「上書き」の segmented button を表示し、保存方法を切り替えます。「上書き」はファイルをドロップする前でも選択できます。安全のため、実行できるのはファイルシステムからドロップしたファイルに限られます。
